#ifndef IMAGE_CONVERTING_H
#define IMAGE_CONVERTING_H
...
inline QImage  cvMatToQImage( const cv::Mat& inMat ) {
  switch ( inMat.type() ) {
    ...
    default:
      qWarning() << "IMGCONV::cvMatToQImage() - cv::Mat image type not handled in switch:" << inMat.type();
      break;
  }
...
inline cv::Mat QImageToCvMat( const QImage& inImage, bool inCloneImageData = true ) {
  switch ( inImage.format() ) {
    ...
    case QImage::Format_RGB32: {
      if ( !inCloneImageData ) {
        qWarning() << "IMGCONV::QImageToCvMat() - Conversion requires cloning so we don't modify the original QImage data";
      }
    ...
    default:
      qWarning() << "IMGCONV::QImageToCvMat() - QImage format not handled in switch:" << inImage.format();
      break;
  }
